 ...
 * @param {object[]}
 * @param [options] {module:dbs-options} - The render options
 * @return {string}
 * @alias module:dbs
 */
function dbs (templateData, options) {
  options = new DbsOptions(options)
  if (skipCache(options)) {
    return generate(templateData, options)
  } else {
    const cached = dbs.cache.readSync([ templateData, options, dbsVersion ])
    if (cached) {
      return cached
    } else {
      return generate(templateData, options)
...
dbs.async = function (templateData, options) {
  options = new DbsOptions(options)
  if (skipCache(options)) {
    return Promise.resolve(generate(templateData, options))
  } else {
    return dbs.cache.read([ templateData, options, dbsVersion ])
      .catch(function () {
        return generate(templateData, options)
      })
  }
...
dbs.cache = new Cache({ dir: path.join(require('os').tmpdir(), 'dbs') })
...
function generate (templateData, options) {
  ...
  options = Object.assign(new DbsOptions(), options)
  ...
  dbs.cache.writeSync([ inputData, inputOptions, dbsVersion ], output)
  return output
}
